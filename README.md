## Combined Poland and Türkiye CERT Threat Feed
A combined domains blocklist from Poland and Türkiye CERT (Computer Emergency Response Team) threat feeds. Provides protection from malware, phishing, and other cyber bad guys. Made available in various formats.

Updated daily.

**All credit resides with these upstream sources:**
* 🇵🇱 [CERT Polska Team](https://cert.pl/en/warning-list)
* ~~ [KAD Przekręty](https://kadantiscam.netlify.app/en)
* 🇹🇷 [USOM CERT Republic of Türkiye](https://www.usom.gov.tr/en/about-us)

#### Download:
* Hosts: https://raw.githubusercontent.com/theouterspaced/polturk-threatfeed/main/hosts.txt
* Domains: https://raw.githubusercontent.com/theouterspaced/polturk-threatfeed/main/domains.txt
* Adblock: https://raw.githubusercontent.com/theouterspaced/polturk-threatfeed/main/adblock.txt
* Wildcard: https://raw.githubusercontent.com/theouterspaced/polturk-threatfeed/main/wildcard.txt

~

Note: This list is provided as-is. I provide no assurances nor accept any responsibility for any outcomes of its use.
